...
class SmartMedicalSearchGlucoMate:
    ...
    def classify_query_type(self, question):
        ...
        # Handle typos and variations for "comment ça va"
        typo_patterns = [
            "common ca va", "comment ca va", "coment ca va", 
            "coment cava", "commnt ca va", "comment sa va"
        ]
        ...
        # Check for casual conversation (including typos)
        if any(casual_word in question_lower for casual_word in casual_indicators):
            return "casual"
        
        # Check for typo variations
        if any(typo in question_lower for typo in typo_patterns):
            return "casual"
...
Respond in {language}:"""
    ...
    def call_bedrock_model(self, prompt):
        try:
            ...
            return f"I'm having trouble connecting to my AI system right now. Please try again in a moment. Error: {str(e)}"
